...
## **Overview**
This field type stores the timestamp indicating when the record was created. Once the record is created, this value remains constant and does not change.
...
This setting determines the format for displaying both the date and time. The default format is **`yyyy-MM-dd HH:mm`**, but you can tailor it to a different format.

While there are several common predefined formats available, you also have the flexibility to select 'Custom' and create your own format adhering to the following guidelines:
...
It is also possible to define a format for the date-time picker.
...
For more information, please refer to the [Query Language Documentation](https://joda-time.sourceforge.net/timezones.html).
...
---|---
Matching operator|**`equals`**<br>UI queries will try to match using the format defined in the default display options of the field.
