...
| **`record`**   | [sys.data.Record]({{<ref "/dev-reference/scripting/sys-data.md#sysdatarecord">}})   | The record involved in the data operation.<br> There is one case where this variable will be empty, which when the event.dataEvent is **`ACTION_PERFORMED`** and the action executed was of type Many records. In this case, as the action is executed over a group of records, it isn’t possible to set a single record.|
